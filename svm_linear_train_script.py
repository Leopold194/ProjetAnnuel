...
for size, categories, c, classifier_type in itertools.product(
    param_grid["sizes"],
    ...
    print(f"\n🧪 Testing size={size}, categories={len(categories)} C={c},  classifier_type={classifier_type}")
    ...
    if acc > best_score:
        best_score = acc
        best_params = (classifier_type, c, size, categories)

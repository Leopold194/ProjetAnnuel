...
fn train(&mut self,x_py: Vec<Vec<f64>>, y_py: Vec<f64>) -> PyResult<()> {
    ...
    let settings = settings.eps_abs(1e-6);
    let settings = settings.eps_rel(1e-6);
...
impl SVMOvR {
pub fn train(&mut self, x: Vec<Vec<f64>>, y: Vec<f64>) -> PyResult<()> {
    let unique_classes: Vec<f64> = {
        let mut s = std::collections::BTreeSet::new();
        for &label in &y { s.insert(label); }
        s.into_iter().collect()
    };
    ...
    for &class_label in &self.classes {
        let binary_labels: Vec<f64> = y.iter().map(|&yi| if yi == class_label { 1.0 } else { -1.0 }).collect();
...
pub fn predict(&self, x_list: Vec<Vec<f64>>) -> PyResult<Vec<f64>> {
...
impl SVMOvO{
    ...
    pub fn train(&mut self, x: Vec<Vec<f64>>, y: Vec<f64>) -> PyResult<()> {
        ...
        classes.sort_by(|a, b| a.partial_cmp(b).unwrap());  // safe if no NaN
        classes.dedup();
    ...
    pub fn predict(&self, x_list: Vec<Vec<f64>>) -> PyResult<Vec<f64>> {
        ...
        for x in &x_list {
            let mut vote_counts = std::collections::HashMap::<f64, usize>::new();

            for &((class_i, class_j), ref svm) in &self.classifiers {
                let score = svm.decision_function(x);
                let winner = if score >= 0.0 { class_i } else { class_j };
                *vote_counts.entry(winner).or_insert(0) += 1;
            }

            let predicted = *vote_counts.iter().max_by_key(|(_, &v)| v).unwrap().0;
